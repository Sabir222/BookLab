...
export default function Home() {
  ...
  // Create different book collections
  const popularBooks = sampleBooks.map(book => ({
    ...book,
  ...
  const newReleases = sampleBooks.slice(2, 7).map(book => ({
    ...book,
    isFavorite: favoriteBooks.has(book.id)
  }));

  const trendingBooks = [...sampleBooks].sort(() => 0.5 - Math.random()).map(book => ({
    ...book,
    isFavorite: favoriteBooks.has(book.id)
  }));

  const fantasyBooks = sampleBooks.filter(book => book.category === "Fantasy" || book.category === "Sci-Fi").map(book => ({
    ...book,
    isFavorite: favoriteBooks.has(book.id)
  }));
  ...
  return (
    <main className="min-h-screen bg-background">
      <div className="container py-8">
        <h1 className="mb-8 text-3xl font-bold">Discover Your Next Read</h1>
